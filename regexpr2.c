...
int nxt, prev = 0;
	...
	if( nxt = probe->stack ) do {
		...
		if( prev ) {
			prevstack = base - prev;
			prevstack->next = base - stack;
		} else
			clone->stack = base - stack;
		prev = base - stack;
...
Status RegExpr (Cmd argv)
{
uint size = argv->count;
Arg arg = argv->arg;
void *expr;
	...
	if( size > sizeof(struct Expr) )
		...
		return scr_intr (argv, ERR_nil);

	if( !regcomp(expr, size, arg->buff, arg->len) )
		return vfree (expr, size), scr_intr (argv, ERR_prep);

	argv->flag = regevaluate (expr, argv->val->buff, argv->val->len);
	vfree (expr, size);
...
int size;
	...
	if( argc < 3 ) {
		printf ("Usage regexpr value expression\n");
		return 1;
	}
	if( argc > 3 )
		size = atoi(args[3]);
	...
	expr = malloc (size);
	expr->val = args[1];
	expr->amt = strlen (args[1]);

	if( !regcomp(expr, size, args[2], strlen(args[2])) )
		printf ("Pattern compilation error\n");
	else if( regevaluate (expr, args[1], strlen(args[1])) )
		printf ("Match in %d steps\n", expr->steps);
	else
		printf ("No match in %d steps\n", expr->steps);
